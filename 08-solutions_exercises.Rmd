...
pacman::p_load("tidyverse", "gapminder", "babynames", "sjPlot")
```
...
```{r chapter 2 exercise 2, eval = FALSE}
#Create the vectors   
...
```{r chapter 3 exercise 3, eval = FALSE}
...
```{r chapter 4 exercise 4, eval = FALSE}
...
```{r chapter3 exercise 1a, eval=FALSE}
ess <- d1 %>% 
...
d1 <- iris %>%
  group_by(Species) %>%
  summarize(PL_average = mean(Petal.Length))
...
```{r chapter 5 exercise 2, eval=FALSE}
...
```{r chapter 5 exercise 3, eval=FALSE}
mnf <- function(a, b, c) {
...
```{r generating data for DA exercises}
